...
public class DynamoDBRepository extends AbstractBackendRepository {
  ...
  public static final String CREATE_TABLE = "schema-repo.dynamo.table.create";
  public static final String REGION_OR_URL = "schema-repo.dynamo.region_or_url";
  public static final String CREDENTIALS_TYPE = "schema-repo.dynamo.credentials.type";
  public static final String PROFILE_CREDENTIALS_NAME =
    "schema-repo.dynamo.credentials.profile.name";
  public static final String STATIC_CREDENTIALS_KEY = "schema-repo.dynamo.credentials.static.key";
  public static final String STATIC_CREDENTIALS_SECRET =
    "schema-repo.dynamo.credentials.static.secret";
  public static final String TABLE_NAME = "schema-repo.dynamo.table.name";
  ...
  @Inject
  public DynamoDBRepository(@Named(CREATE_TABLE) boolean create,
    @Named(REGION_OR_URL) String regionOrUrl,
    @Named(CREDENTIALS_TYPE) String credentialsType,
    @Named(PROFILE_CREDENTIALS_NAME) String profileName,
    @Named(STATIC_CREDENTIALS_KEY) String staticKey,
    @Named(STATIC_CREDENTIALS_SECRET) String staticSecret,
    @Named(TABLE_NAME) String tableName,
    ValidatorFactory factory) {
    this(factory, getDynamo(credentials(credentialsType, profileName, staticKey, staticSecret),
      regionOrUrl),
      tableName);
    if (create) {
      try {
        ...
        CreateTableRequest createRequest = this.mapper.generateCreateTableRequest(SubjectSchema
          .class);
        createRequest.setProvisionedThroughput(new ProvisionedThroughput(1l, 1l));
        this.dynamo.createTable(createRequest);
      }
    }
  }

  private static AWSCredentialsProvider credentials(String credentialsType, String profileName,
    String staticKey, String staticSecret) {
    switch (credentialsType.toLowerCase()) {
      case "default":
      ...
      default:
        throw new IllegalArgumentException("Unsupported credential type: " + credentialsType);
    }
  }

  private static AmazonDynamoDB getDynamo(AWSCredentialsProvider credentials,
    String regionOrUrl) {
